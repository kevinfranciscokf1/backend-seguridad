...
router.get('/tanques', (req, res) => {
    db.query('SELECT * FROM tanques', (err, results) => {
        if (err) {
...
router.post('/tanques', (req, res) => {
    const { nombre, descripcion } = req.body; 
    const sql = 'INSERT INTO tanques (codigo, descripcion, serie, ubicacion) VALUES (?, ?, ?, ?)';
...
router.put('/tanques/:tanquesId', (req, res) => {
    const equipoId = req.params.equipoId;
    const { nombre, descripcion } = req.body; // Datos actualizados.
    const sql = 'UPDATE tanques SET nombre = ?, descripcion = ? WHERE id = ?';
...
router.delete('/tanques/:tanquesId', (req, res) => {
    const equipoId = req.params.equipoId;
    const sql = 'DELETE FROM tanques WHERE id = ?';
